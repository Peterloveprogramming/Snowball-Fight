...
public class AvatarInputConverter : MonoBehaviour
{
    ...
    void Update()
    {
       ...
       //Hands sync
        ...
        AvatarHand_Right.rotation = Quaternion.Lerp(AvatarHand_Right.rotation, XRHand_Right.rotation  * Quaternion.Euler(handRotationOffset), 0.5f);



        AvatarHand_Left.position = Vector3.Lerp(AvatarHand_Left.position, XRHand_Left.position,0.5f);

        AvatarHand_Left.rotation = Quaternion.Lerp(AvatarHand_Left.rotation, XRHand_Left.rotation  * Quaternion.Euler(handRotationOffset), 0.5f);
    }
}